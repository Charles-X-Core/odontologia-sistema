...
exports.crear = async (req, res) => {
  ...
  const existente = await db.prepare('SELECT id FROM historias_clinicas WHERE paciente_id = ?').get(paciente_id);
  if (existente) {
...
exports.obtenerPorPaciente = async (req, res) => {
  const historia = await db.prepare('SELECT * FROM historias_clinicas WHERE paciente_id = ?').get(req.params.pacienteId);
