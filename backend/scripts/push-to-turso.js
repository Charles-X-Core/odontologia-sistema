...
const schema = [
  `CREATE TABLE IF NOT EXISTS usuarios (
    ...
    titulo TEXT DEFAULT 'C.D Odontologia',
  ...
  `CREATE TABLE IF NOT EXISTS pacientes (
    ...
    lugar_procedencia TEXT DEFAULT '',
    grado_instruccion TEXT DEFAULT '',
    ocupacion TEXT,
  ...
  `CREATE TABLE IF NOT EXISTS historias_clinicas (
    ...
    created_at TEXT DEFAULT (datetime('now')),
    alergia_medicamentos TEXT DEFAULT '',
    propension_hemorragias TEXT DEFAULT '',
    complicaciones_anestesia TEXT DEFAULT '',
    presion_arterial_medicacion TEXT DEFAULT '',
    cardiopatias_personales TEXT DEFAULT '',
    cardiopatias_familiares TEXT DEFAULT '',
    diabetes_personal TEXT DEFAULT '',
    diabetes_familiar TEXT DEFAULT '',
    hepatitis TEXT DEFAULT '',
  ...
  `CREATE TABLE IF NOT EXISTS consultas (
    ...
    tiempo_enfermedad TEXT DEFAULT '',
    signos_sintomas TEXT DEFAULT '',
    relato_cronologico TEXT DEFAULT '',
    funciones_biologicas TEXT DEFAULT '',
    signos_vitales TEXT DEFAULT '{}',
    examen_clinico_general TEXT DEFAULT '',
    evaluacion_odontoestomatologica TEXT DEFAULT '',
    diagnostico_lista TEXT DEFAULT '[]',
  ...
  `CREATE TABLE IF NOT EXISTS tratamientos (
    ...
    consulta_id INTEGER,
    fecha TEXT NOT NULL,
    pieza_dental TEXT DEFAULT '',
    procedimiento_realizado TEXT NOT NULL,
    costo_total REAL DEFAULT 0,
    monto_a_cuenta REAL DEFAULT 0,
  ...
  `CREATE TABLE IF NOT EXISTS whatsapp_log (
    ...
    usuario_id INTEGER,
    delivery_status TEXT DEFAULT 'enviado',
  ...
  `CREATE TABLE IF NOT EXISTS whatsapp_batch (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    nombre TEXT DEFAULT '',
  ...
  `CREATE TABLE IF NOT EXISTS citas (
    ...
    motivo_editado TEXT DEFAULT '',
    estado TEXT DEFAULT 'pendiente',
    notas TEXT DEFAULT '',
    recordatorio_enviado INTEGER DEFAULT 0,
    consulta_id INTEGER,
    asistio_confirmed_at TEXT,
    created_at TEXT DEFAULT (datetime('now')),
  ...
  `CREATE TABLE IF NOT EXISTS importaciones_historial (
    ...
    pacientes_creados INTEGER DEFAULT 0,
    pacientes_duplicados INTEGER DEFAULT 0,
    consultas_creadas INTEGER DEFAULT 0,
    tratamientos_creados INTEGER DEFAULT 0,
    pagos_creados INTEGER DEFAULT 0,
...
const seedData = [
  {
    sql: `INSERT OR IGNORE INTO whatsapp_plantillas (nombre, categoria, asunto, cuerpo) VALUES (?, ?, ?, ?)`,
    args: ['Receta Medica', 'salud', 'Receta Medica', 'Buenos dias {nombre_completo}, le comparto su receta medica:\n\n{medicamentos}\n\nIndicaciones: {indicaciones}\n\nSi tiene alguna duda, no dude en comunicarse.\n_Clinica Dental Pro - Clinica Odontologica_'],
  },
  {
    sql: `INSERT OR IGNORE INTO whatsapp_plantillas (nombre, categoria, asunto, cuerpo) VALUES (?, ?, ?, ?)`,
    args: ['Plan de Tratamiento', 'tratamiento', 'Plan de Tratamiento', 'Buenos dias {nombre_completo}, le compartimos su plan de tratamiento:\n\n{tratamientos_lista}\n\nTotal: S/{costo_total}\nSaldo pendiente: S/{saldo_pendiente}\n\nAgende su proxima cita: 982-890-328\n_Clinica Dental Pro - Clinica Odontologica_'],
  },
  {
    sql: `INSERT OR IGNORE INTO whatsapp_plantillas (nombre, categoria, asunto, cuerpo) VALUES (?, ?, ?, ?)`,
    args: ['Recordatorio de Pago', 'pago', 'Recordatorio de Pago', 'Buenos dias {nombre_completo}, le recordamos que tiene un saldo pendiente de:\n\nS/{saldo_pendiente}\n\nPuede realizar su pago en clinica o comunicarse para coordinar una fecha.\n\n_Clinica Dental Pro - Clinica Odontologica_'],
  },
  {
    sql: `INSERT OR IGNORE INTO whatsapp_plantillas (nombre, categoria, asunto, cuerpo) VALUES (?, ?, ?, ?)`,
    args: ['Proxima Cita', 'cita', 'Proxima Cita', 'Buenos dias {nombre_completo}, le recordamos su proxima cita:\n\nFecha: {fecha_cita}\nHora: {hora_cita}\nProcedimiento: {procedimiento}\n\nSi necesita reprogramar, comuniquese con nosotros.\n_Clinica Dental Pro - Clinica Odontologica_'],
  },
  {
    sql: `INSERT OR IGNORE INTO whatsapp_plantillas (nombre, categoria, asunto, cuerpo) VALUES (?, ?, ?, ?)`,
    args: ['Bienvenida', 'marketing', 'Bienvenida', 'Hola {nombre_completo}, bienvenido(a) a Clinica Dental Pro - Clinica Odontologica!\n\nEstamos encantados de atenderle. Si tiene alguna consulta, no dude en escribirnos.\n\nTelefono: 982-890-328\n_Clinica Dental Pro - Clinica Odontologica_'],
  },
  {
    sql: `INSERT OR IGNORE INTO whatsapp_plantillas (nombre, categoria, asunto, cuerpo) VALUES (?, ?, ?, ?)`,
    args: ['Seguimiento', 'salud', 'Seguimiento Post-Tratamiento', 'Buenos dias {nombre_completo}, como se encuentra despues de su tratamiento?\n\nSi tiene alguna molestia o consulta, estamos para ayudarle.\n\n_Clinica Dental Pro - Clinica Odontologica_'],
  },
  {
    sql: `INSERT OR IGNORE INTO whatsapp_plantillas (nombre, categoria, asunto, cuerpo) VALUES (?, ?, ?, ?)`,
    args: ['Confirmacion de Cita', 'cita', 'Confirmacion de Cita', 'Buenos dias {nombre_completo}, tiene una cita programada para:\n\nFecha: {fecha_cita}\nHora: {hora_cita}\n\nPor favor confirme su asistencia respondiendo SI o NO.\n_Clinica Dental Pro - Clinica Odontologica_'],
  },
  {
    sql: `INSERT OR IGNORE INTO whatsapp_plantillas (nombre, categoria, asunto, cuerpo) VALUES (?, ?, ?, ?)`,
    args: ['Feliz Cumpleanos', 'marketing', 'Feliz Cumpleanos!', 'Feliz cumpleanos {nombre_completo}!\n\nDe parte de todo el equipo de Clinica Dental Pro le deseamos lo mejor en este dia tan especial.\n\nTelefono: 982-890-328\n_Clinica Dental Pro - Clinica Odontologica_'],
  },
  {
    sql: `INSERT OR IGNORE INTO whatsapp_plantillas (nombre, categoria, asunto, cuerpo) VALUES (?, ?, ?, ?)`,
    args: ['Recordatorio de Limpieza', 'salud', 'Recordatorio de Limpieza Dental', 'Buenos dias {nombre_completo}, ya han pasado varios meses desde su ultima limpieza dental.\n\nEs recomendable realizarse una limpieza cada 6 meses para mantener su salud bucal.\n\nAgende su cita: 982-890-328\n_Clinica Dental Pro - Clinica Odontologica_'],
  },
  {
    sql: `INSERT OR IGNORE INTO whatsapp_plantillas (nombre, categoria, asunto, cuerpo) VALUES (?, ?, ?, ?)`,
    args: ['Saldo a Favor', 'pago', 'Saldo a Favor', 'Buenos dias {nombre_completo}, le informamos que cuenta con un saldo a favor de:\n\nS/{saldo_a_favor}\n\nEste credito puede ser utilizado en su proximo tratamiento.\n\n_Clinica Dental Pro - Clinica Odontologica_'],
  },
  {
    sql: `INSERT OR IGNORE INTO whatsapp_plantillas (nombre, categoria, asunto, cuerpo) VALUES (?, ?, ?, ?)`,
    args: ['Custom', 'otro', '', '{mensaje_personalizado}'],
  },
];
...
async function pushSchema() {
  console.log('[Turso] Conectando a:', process.env.TURSO_URL);

  try {
    // Test connection
    const test = await client.execute('SELECT 1 as ok');
    console.log('[Turso] Conexion OK');

    // Create tables
    console.log('[Turso] Creando tablas...');
    ...
    for (const { sql, args } of seedData) {
      await client.execute({ sql, args });
    }
    console.log('[Turso] Plantillas insertadas OK');

    // Seed config
    console.log('[Turso] Insertando configuracion...');
    for (const [k, v, d] of configDefaults) {
      await client.execute({
        sql: 'INSERT OR IGNORE INTO whatsapp_config (clave, valor, descripcion) VALUES (?, ?, ?)',
        args: [k, v, d],
      });
    }
    console.log('[Turso] Configuracion insertada OK');

    // Verify tables
    const tables = await client.execute("SELECT name FROM sqlite_master WHERE type='table' ORDER BY name");
    console.log('[Turso] Tablas en la nube:', tables.rows.map(r => r.name).join(', '));

    console.log('\n[Turso] MIGRACION COMPLETADA EXITOSAMENTE');
  } catch (error) {
    console.error('[Turso] ERROR:', error.message);
    process.exit(1);
  }
}
